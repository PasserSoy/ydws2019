$(function () {
  ...
  POST('/sys/company/queryStaticThree.v1', {
    ...
    if (res.code == 0) {
      ...
      var opt = '';
      res.forEach(x => {
        opt += `<option data-stid="${x.id}" value="${x.id}">${x.companyName}</option>`;
      });
      ...
      //       _jcz += `<option data-stid="${x.id}" value="${x.id}">${x.stationName}</option>`;
      //     });
      //   };
      // };
      ...
      $('select#monitor').change(function () {
        ...
        var _c2 = '<option data-stid="allst" value="">全部</option>'; // 母线的内容
        if (_index1 > -1 && res[_index].monitorStationDtos[_index1].linesOnMasterDtos != undefined && res[_index].monitorStationDtos[_index1].linesOnMasterDtos.length > 0) {
          res[_index].monitorStationDtos[_index1].linesOnMasterDtos.forEach(x => {
            _c2 += `<option data-stid="${x.id}" value="${x.id}">${x.masterName}</option>`;
          });
        };
  ...
  function mychange(){
    ...
    $('#company').change();
    // 通过保存的下拉框设置当前选中监测站
    $(`#monitor option[data-stid=${sessionStorage.stmonitorstation}]`).attr('selected',true);
    $('#monitor').change();
    // 通过保存的下拉框设置当前选中母线
    $(`#bus option[data-stid=${sessionStorage.stgeneratrix}]`).attr('selected',true);
    $('#bus').change();
  }
  ...
  $('.time input[type=radio]').change(function () {
    ...
    if (_id == 'week') { // 周报
      ...
      if((mydate.getMonth() + 1)==12) eMonth = mydate.getFullYear() + '-12-31';
  ...
  var url = '/api/count/getStatisticsData'; // 页面数据请求接口
  ///////点击查询
  ...
  function initData() { // 初始化页面
    ...
    // console.log(toData)
    // 下载地址
    var downloadUrl = rooturl+url + '?token=' + sessionStorage.token + '&timeStart=' + timeStart + '&timeEnd=' + timeEnd + '&masterFlag=' + masterFlag + '&voltageFlag=' + voltageFlag + '&temperFlag=' + temperFlag + '&userId=' + sessionStorage.id + '&companyId=' + companyId + '&monitorStationId=' + monitorStationId + '&masterId=' + masterId + '&isDown=Y';
    $('#download').attr('href', downloadUrl);
    POST(url, toData, function (res) {
  ...
  $('.sort i').click(function (e) { // 点击箭头排序表格
    e.stopPropagation();
    var _t = $(this),
      className = _t.attr('class'),
      pTable = _t.parents('.mytable').data('table'),
      phase = _t.parents('th').hasClass('phase');
  ...
  $('.sort').click(function () { // 点击表头排序
    var _t = $(this),
      _active = _t.find('i.active'); // _active 当前的排序箭头
  ...
  // function myAjax(url,data,callback,bef,comp){// ajax方法
  ...
  function draw(data) { // 描表
    ...
    if (dataCopy.maxValueEric.length > 0) {
      dataCopy.maxValueEric.forEach(x => { // 遍历生成对应的td结构
        x.phase.forEach((y, i) => { // 遍历母线下的相位数据
          ...
          switch (i) {
            case 0:
              ts = `<td rowspan="3" class="name">${x.masterName}</td><td class="A">A相</td>`;
              break;
            case 1:
              ts = `<td class="B">B相</td>`;
              break;
    ...
    if (dataCopy.maxValueVoltage.length > 0) {
      dataCopy.maxValueVoltage.forEach(x => { // 遍历生成对应的td结构
        x.phase.forEach(y => y.temper = fixNum(y.temper)); // 避免渲染时倒序
        x.phase.forEach((y, i) => { // 遍历过电压保护器下的相位数据
          ...
          switch (i) {
            case 0:
              ts = `<td rowspan="3" class="name">${x.pointName}</td><td rowspan="3">${x.type}</td><td class="A">A相</td>`;
              break;
            case 1:
              ts = `<td class="B">B相</td>`;
              break;
      ...
      gdyTr = `<tr class="white"><td colspan="8">暂无数据</td></tr>\n`;
    };
    ...
    // 渲染温度采集器表格
    var wdTr = '';
    if (dataCopy.regroupMaxValueTemper.length > 0) {
      dataCopy.regroupMaxValueTemper.forEach(x => { // 遍历新的数组
        // wdTr += `<tr class="firstTr"><td class="name">${x.positionName.slice(0,-2)}</td><td>${x.type}</td><td class="A">A相</td><td>${x.a}</td><td class="B">B相</td><td>${x.b}</td><td class="C">C相</td><td>${x.c}</td></tr>`;
        wdTr += `<tr class="firstTr"><td class="name">${x.positionName}</td><td>${x.type}</td><td class="A">A相</td><td>${x.temper_a}</td><td class="B">B相</td><td>${x.temper_b}</td><td class="C">C相</td><td>${x.temper_c}</td></tr>`;
      });
    } else {
      wdTr = `<tr class="white"><td colspan="8">暂无数据</td></tr>\n`;
    };
    $('.wdTable tbody').empty().append(wdTr);
    // ./渲染温度采集器表格

    // 渲染报警统计表格
    $('.bjTable tbody .sumCount').text(dataCopy.sumCount != undefined ? dataCopy.sumCount : 0);
  ...
  function dealData(data) { // 处理数据
    ...
    data.regroupMaxValueTemper.forEach(x => {
      ...
      x.temper_b = x.temper_b != undefined ? x.temper_b : '';
      x.temper_c = x.temper_c != undefined ? x.temper_c : '';
    });
    // ./处理温度采集器数据 maxValueTemper
  ...
  ///// rowspan=3 hover
  $('body').on('mouseenter', 'td[rowspan=3]', function () {
    var _t = $(this),
      _tr = _t.parent('tr'),
      _td1 = _t.siblings('td'),
      _td2 = _tr.next('tr').find('td'),
      _td3 = _tr.next('tr').next('tr').find('td');
    _t.addClass('color');
    _td1.addClass('color');
    _td2.addClass('color');
    _td3.addClass('color');
  });
  $('body').on('mouseleave', 'td[rowspan=3]', function () {
    var _t = $(this),
      _tr = _t.parent('tr'),
      _td1 = _t.siblings('td'),
      _td2 = _tr.next('tr').find('td'),
      _td3 = _tr.next('tr').next('tr').find('td');
    _t.removeClass('color');
    _td1.removeClass('color');
    _td2.removeClass('color');
    _td3.removeClass('color');
  });

  $('body').on('mouseenter', 'td:not([rowspan])', function () {
    $(this).addClass('color');
    $(this).siblings('td:not([rowspan])').addClass('color');
  });
  $('body').on('mouseleave', 'td:not([rowspan])', function () {
    $(this).removeClass('color');
    $(this).siblings('td:not([rowspan])').removeClass('color');
  });
})
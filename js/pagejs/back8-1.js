...
if(getPropetyVal('index')=='myIndex')$('.navigation_l #first_li .back').attr('href','myIndex.html?v=1.6.22');
$('#fname').text(fname);
...
var faultUrl = '/eric/getFaultOverVoltage.v2'; // 折线图请求地址 2019/05/23 修改需求 改为v2
switch (type) {
  case 10:
...
// echarts.connect([myChart_mains_one, myChart_mains_one_1,myChart_mains_two,myChart_mains_two_1]);
...
if(type==20){// 过热处理 分为过电压保护器、温度采集器
  ...
  function Statistics_btn(xAxis_Wave_UA, xAxis_Wave_UB, xAxis_Wave_UC, xAxis_Wave_U0, xAxis_Wave_IA, xAxis_Wave_IB, xAxis_Wave_IC, xAxis_Wave_IX) {
    var indexDom = `<div class="echartList"> <div data-type="dy"></div> <div data-type="lxdy"></div> <div data-type="dl"></div> <div data-type="lxdl"></div> </div>`;
    window.parent.$('body').append(indexDom);// 父元素加dom结构
    var times = 500,dy='',lxdy='',dl='',lxdl='';
    // 电压波形
    option_one = {
      color: ['#FFCD2D', '#14AE68', '#FB1734', '#333'],
      title: {
      ...
      tooltip: {
        ...
        formatter: function (params) {
          ...
          dy = `<div class="echartData"><p class="tit">电压波形</p>${text}</div>`;
          window.parent.$('.echartList [data-type=dy]').empty().append(dy);
    ...
    myChart_mains_one.setOption(option_one);
    $('.echartM').on('mouseout',function(){
      sessionStorage.point='';
      window.parent.$('.echartData').hide();
    });
    // 零序电压
    option_one_1 = {
      color: ['#333'],
      ...
      tooltip: {
        ...
        formatter: function (params) {
          ...
          lxdy = `<div class="echartData"><p class="tit">零序电压</p>${text}</div>`;
          window.parent.$('.echartList [data-type=lxdy]').empty().append(lxdy);
          text = `<div class="echartData">${text}</div>`;
          return text;
        },
        axisPointer:{
    ...
    option_two = {
      ...
      tooltip: {
        ...
        formatter: function (params) {
          ...
          dl = `<div class="echartData"><p class="tit">电流波形</p>${text}</div>`;
          window.parent.$('.echartList [data-type=dl]').empty().append(dl);
          text = `<div class="echartData">${text}</div>`;
          return text;
        },
        axisPointer:{
    ...
    option_two_1 = {
      ...
      tooltip: {
        ...
        formatter: function (params) {
          ...
          lxdl = `<div class="echartData"><p class="tit">零序电流</p>${text}</div>`;
          window.parent.$('.echartList [data-type=lxdl]').empty().append(lxdl);
          text = `<div class="echartData">${text}</div>`;
          return text;
        },
        axisPointer:{
...
function queryFaultGroundInfoList(data) {// 报警处理

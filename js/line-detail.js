    ...
    linesOnMasterSortList.forEach(x=>{
      ...
      linesOnBranchDtos.forEach(x=>{// 遍历支线数组
        ...
        if(x.branchType=='1'){
          ...
          if(!_table==''){
            itable = `<li><h6 class="b-name">${x.branchName}</h6><h5 class="ctnum">CT绕组数:${x.sensorNumber}</h5><div class="table">${_table}</div></li>`;// 表
          };
          ...
          if(!_table==''){
            otable += `<li><h6 class="b-name">${x.branchName}</h6><h5 class="ctnum">CT绕组数:${x.sensorNumber}</h5><div class="table">${_table}</div></li>`;// 表
          };
          ...
          if(!_table==''){
            ctable = `<li><h6 class="b-name">${x.branchName}</h6><h5 class="ctnum">CT绕组数:${x.sensorNumber}</h5><div class="table">${_table}</div></li>`;// 表
          };
      ...
      +`" data-err="${x.isProblem}" data-monitorFlag="${x.monitorFlag}" data-masterName="${x.masterName}">
        <h3 class="tit"><span class="kv">${x.hitchType} KV</span><span class="linename">${x.masterName}</span><div class="linep"></div></h3>
      ...
      ///////////渲染线路表
      var _myLineTable='';
      if(ctable!=''||itable!=''||otable!=''){//ctable 联络柜表格；itable 进线柜表格；otable 出线柜表格
        var fault = x.isProblem=='Y'?'fault':'';// 是否为故障母线
        _myLineTable = `<li><h5 class="m-name ${fault}">${x.masterName}</h5>
        <ul class="branch-collect"> ${ctable} ${itable} ${otable}</ul></li>`;
      };
